            ...
            return [
                ...
                "products" => $accountsByProduct->map(function ($accounts, $productId) use ($asOf) {
                    $accountsRemapped = $accounts->map(function ($account) use ($asOf) {
                        ...
                        // Exclude accounts explicitly based on loan status (if necessary)
                        if ($account->loan_status === LoanAccount::LOAN_WRITEOFF) {
                            return null; // Exclude Write-Off accounts explicitly
                        }
                        ...
                        // Ensure null accounts are excluded
                        return $account !== null && $account['principal_balance_as_of'] > 0;
                    });
                    ...
                    return [
                       ...
                       "all" => [
                                'count' => $accountsRemapped->count(), // Count only filtered and valid accounts
                                'total_loan_amount' => $accountsRemapped->sum('loan_amount'),
                                'amount' => $accountsRemapped->sum('principal_balance_as_of'),
                                'rate' => 0,
                            ],
                        "delinquent" => [
                                ...
                                'rate' => $allAmount ? round($delinquentAccounts->sum('principal_balance_as_of') / $allAmount * 100, 2) : 0,
                            ],
                        "pastdue" => [
                                ...
                                'rate' => $allAmount ? round($pastDueAccounts->sum('principal_balance_as_of') / $allAmount * 100, 2) : 0,
                            ],
