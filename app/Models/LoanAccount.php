...
class LoanAccount extends Model
{
    ...
    {
        $amortization = null;
        $currentAmort = $account->with('amortizations', function ($query) {
            $query->where('status', 'open')
                ->orderBy('id')
                ->first();
        })->accountId();
        $amortization = $currentAmort->amortizations;
        if (!$amortization) {

            $nextAmort = $account->with('amortizations', function ($query) {
                $query->orderBy('id', 'DESC')
                    ->first();
            })->accountId();
            $amortization = $nextAmort->amortizations;
        }
        return $amortization;
    }
    /*     private function setAmortizationStatus($amortStatus, $prevAmort, $transactionDateNow)
    {
        $status = null;
        $lastAmortization = $prevAmort->amortizations->first();
        $lastAmortStatus = null;

        if ($lastAmortization) {
            $lastAmortStatus = $lastAmortization->status;
        }
        $amortPayment = $prevAmort->payments->last();
        if ($amortPayment) {
            if ($amortPayment->short_principal || $amortPayment->short_interest) {
    ...
    {
        $isDelinquent = false;
        $currentAmortPrincipal = $amortization->principal + $amortization->short_principal;
        $isDelinquent = $dayDiff > 0 && $amortization->advance_principal < $currentAmortPrincipal ? !$isDelinquent : $isDelinquent;
        return $isDelinquent;
    }

    /*     private function setAmortizationDetails($account, $amortization, $transactionDateNow, $prevAmort = null, $payment, $amortizationStatus, $lastPaidAmort)
    {
        ...
        $amortization->delinquent = $this->setDelinquentAmortization($account, $amortization->advance_principal, $prevAmort, $amortization, $payment);
        ...
        if ($isDelinquent) {
            ...
            if ($transactionDateNow > $amortization->amortization_date) {
                LoanAccount::find($this->loan_account_id)->update(['payment_status' => LoanAccount::PAYMENT_DELINQUENT]);
            }
        } else {
            $amortization->short_principal = $amortization->delinquent['principal'] - (in_array($prevAmort->amortizations ? $prevAmort->id : $amortization->id, $amortization->delinquent['ids'])
                ? $amortization->principal : 0);
            $amortization->short_interest = $amortization->delinquent['interest'] - (in_array($prevAmort->amortizations ? $prevAmort->id : $amortization->id, $amortization->delinquent['ids']) ?
                $amortization->interest : 0);
        }
    ...
    private function getLoanNextAmortization($amortization, $account, $transactionDateNow)
    {
        if ($account && $amortization) {
            $id = $amortization->id;
            $nextAmort = $account->without(['payments', 'branch'])->with('amortizations', function ($query) use ($id) {
                $query->where('status', 'open')
                    ->first();
    ...
    private function setNextAmort($isPaid)
    {
        $amortization = new Amortization();
        if ($isPaid) {
            return $amortization->getNextAmortization($this->loan_account_id);
        }
    }
    ...
    private function getLoanAmortization()
    {
        $amortization = new Amortization();
        $transactionDateNow = transactionDate($this->branch->branch_id);
        $amortization = $amortization->getCurrentAmortizationByAccount($this->loan_account_id, $transactionDateNow);
        if ($amortization) {
            ...
            $accPayments = $amortization->getAmortizationPayments($amortization->id);
            $lastPayment = $accPayments->last() ?? null;
            ...
            $prevAmort = $amortization->getPrevAmort($this->loan_account_id, $amortization->id);
            $prevAmortStatus = $prevAmort ? $prevAmort->status : null;


            $delinquentAmortization = $amortization->getDelinquentAmortization($this->loan_account_id, $lastPaidAmort, $amortization->id);
            $amortization->delinquent = $this->setDelinquentAmortization($delinquentAmortization, $prevAmort, $amortization->advance_principal);
            ...
            $amortizationPayments = $this->getPayments($amortization->id);
            ...
            if ($isMonthlyPayment) {
                $monthlySched = $amortization->amortization_date->startOfMonth();
                $endOfMonth = $transactionDateNow->endOfMonth();
                if ($transactionDateNow > $amortization->amortization_date->endOfMonth()) {
                    ...
                    $amortization = $amortization->getNextAmortization($this->loan_account_id);
                }
            } else {
                $dayDiff = $amortization->amortization_date->diffInDays($transactionDateNow, false);
                if ($transactionDateNow > $amortization->amortization_date) {
            ...
            /*  if ($isPaid) {
                $currentAmortization = $amortization->getNextAmortization($this->loan_account_id);
 ...
 */
    ...
    /* public function setDelinquentAmortization($account, $advancePrincipal = 0, $prevAmort, $currentAmort, $payment)
    {
        ...
        $totalPenalty = 0;
        $balance = 0;
        $lastAmortizationStatus = null;

        if ($currentAmort) {
            $lastAmort = $prevAmort->amortizations->first();
            $lastAmortizationStatus = $lastAmort ? $lastAmort->status : null;

            if ($lastAmort && $lastAmort->status === self::AMORTIZATION_DELINQUENT) {
                if ($delinquents) {
                    foreach ($delinquents as $delinquent) {
                        $ids[] = $delinquent->id;
    ...
    /*    public function setDelinquentAmortization($account, $advancePrincipal = 0, $lastPaidAmort, $currentAmort, $prevAmort)
    {
        $delinquents = $this->getDelinquentAmortization($account, $lastPaidAmort, $currentAmort);
        ...
        $balance = 0;
        if ($prevAmort) {
            if ($prevAmort->status === self::AMORTIZATION_DELINQUENT) {
    ...
    public function setDelinquentAmortization($delinquents, $prevAmort, $advancePrincipal = 0)
    {
    ...
    public function getDelinquentAmortization($account, $lastPaidAmort, $currentAmort)
    {
        $amortizations = [];
        if ($lastPaidAmort) {
    ...
    {
        $accPayments = LoanAccount::select('loan_account_id')
        ...
        return $accPayments->payments;

        /*         $account = LoanAccount::with('payments', function ($query) use ($amortizationId) {
 ...
 */
    ...
    public function getPayments($amortizationId)
    {
        return Payment::where('amortization_id', $amortizationId)->get();
        /*         $accountPayments = $account->without('branch')->with('amortizations', function ($query) use ($amortizationId) {
            $query->where('id', $amortizationId);
        })->accountId(); */
    }
