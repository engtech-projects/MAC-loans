...
class BorrowerController extends BaseController
{
    ...
    public function borrowerList($branchId, Request $request)
    {
        ...
        foreach ($brrwrs as $bKey => $borrower) {

            $totalAccounts = count($borrower->accounts);
            if ($totalAccounts > 0) {
                $totalPaid = 0;
                foreach ($borrower->accounts as $accKey => $account) {
                    $remainBalance = $account->remainingBalance();
                    if ($remainBalance["memo"]["balance"] === 0) {
                        $totalPaid++;
                    }
                }
                if ($totalPaid == $totalAccounts) {
                    $brrwrs->forget($bKey);
                }
            } else {
        ...
        return  $this->sendResponse(array_values($brrwrs->toArray()), 'Borrowers');
        ...
        if (isset($request->onlyUnpaid)) {
            $borrowers = $borrowers->where('loan_status', '!=', LoanAccount::PAYMENT_PAID)->get();
            foreach ($borrowers as $bKey => $borrower) {
                $accounts = $borrower->borrowerAccounts()->toArray();
            }
            $collection = collect($borrowers);
